...
data class GitRepositoryWrapperResponse(
...
fun GitRepositoryWrapperResponse.toModel(): GitRepositoryWrapperModel {
    return GitRepositoryWrapperModel(
        totalCount = totalCount ?: 0,
        items = items?.map { item ->
            GitRepositoryWrapperModel.GitRepositoryDataModel(
                id = item.id ?: 0,
                name = item.name ?: "",
                ...
                owner = GitRepositoryWrapperModel.GitRepositoryOwnerModel(
                    id = item.owner?.id ?: 0,
